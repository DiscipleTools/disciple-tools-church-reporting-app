<?php
/**
 * Plugin Name: Disciple.Tools - Reporting App
 * Plugin URI: https://github.com/DiscipleTools/disciple-tools-reporting-app
...
/**
 * Gets the instance of the `DT_Reporting_App` class.
 *
 * @since  0.1
 * @access public
 * @return object|bool
...
function dt_reporting_app() {
    ...
    if ( $is_theme_dt && version_compare( $version, $dt_reporting_app_required_dt_theme_version, "<" ) ) {
        add_action( 'admin_notices', 'dt_reporting_app_hook_admin_notice' );
...
class DT_Reporting_App {
    ...
    public function plugin_description_links( $links_array, $plugin_file_name, $plugin_data, $status ) {
        if ( strpos( $plugin_file_name, basename( __FILE__ ) ) ) {
            ...
            $links_array[] = '<a href="https://github.com/DiscipleTools/disciple-tools-reporting-app">Github Project</a>';
        }
    ...
    public static function deactivation() {
        ...
        delete_option( 'dismissed-disciple-tools-reporting-app' );
    }
    ...
    public function i18n() {
        $domain = 'disciple-tools-reporting-app';
    ...
    public function __toString() {
        return 'disciple-tools-reporting-app';
    }
    ...
    public function __call( $method = '', $args = array() ) {
        _doing_it_wrong( "dt_reporting_app::" . esc_html( $method ), 'Method does not exist.', '0.1' );
        unset( $method, $args );
        return null;
    }
}
...
register_activation_hook( __FILE__, [ 'DT_Reporting_App', 'activation' ] );
register_deactivation_hook( __FILE__, [ 'DT_Reporting_App', 'deactivation' ] );


if ( ! function_exists( 'dt_reporting_app_hook_admin_notice' ) ) {
    function dt_reporting_app_hook_admin_notice() {
        global $dt_reporting_app_required_dt_theme_version;
        $wp_theme = wp_get_theme();
        ...
        if ( $wp_theme->get_template() === "disciple-tools-theme" ){
            $message .= ' ' . sprintf( esc_html( 'Current Disciple Tools version: %1$s, required version: %2$s' ), esc_html( $current_version ), esc_html( $dt_reporting_app_required_dt_theme_version ) );
        }
        // Check if it's been dismissed...
        if ( ! get_option( 'dismissed-disciple-tools-reporting-app', false ) ) { ?>
            <div class="notice notice-error notice-disciple-tools-reporting-app is-dismissible" data-notice="disciple-tools-reporting-app">
                <p><?php echo esc_html( $message );?></p>
            </div>
            <script>
                jQuery(function($) {
                    $( document ).on( 'click', '.notice-disciple-tools-reporting-app .notice-dismiss', function () {
                        $.ajax( ajaxurl, {
                            type: 'POST',
                            data: {
                                action: 'dismissed_notice_handler',
                                type: 'disciple-tools-reporting-app',
    ...
    if ( is_admin() && !( is_multisite() && class_exists( "DT_Multisite" ) ) || wp_doing_cron() ){
        ...
        if ( class_exists( 'Puc_v4_Factory' ) ){
            Puc_v4_Factory::buildUpdateChecker(
                'https://raw.githubusercontent.com/DiscipleTools/disciple-tools-reporting-app/master/version-control.json',
                __FILE__,
